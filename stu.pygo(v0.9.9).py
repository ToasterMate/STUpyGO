    ...
    for x in range(wordamount):
        ...
        def getword(number):
            ...
            if correctanswer == number:
                return itemlist[activeitem + 1]
            else:
                while True:
                    tempitem = random.randint(0, wordamount*2)
                    if tempitem % 2 == 1 and tempitem <= len(itemlist):
                        return itemlist[tempitem]
        choice1 = getword(1)
        choice2 = getword(2)
        choice3 = getword(3)
        choice4 = getword(4)
        ...
        try:
            print(f"1: {choice1}2: {choice2}3: {choice3}4: {choice4}")
            player_input = int(input(f"{itemlist[activeitem]}\nWhich number is correct? "))
            if player_input in choices:
                if choices[player_input] == itemlist[activeitem + 1]:
                    print("correct!")
                    time.sleep(2)
                    activeitem += 2
                else:
                    print(f"false: {itemlist[activeitem + 1]}")
                    time.sleep(2)
                    activeitem += 2
            else:
